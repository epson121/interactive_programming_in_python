...
def draw(canvas):
    ...
    canvas.draw_text("Score", [50, 120], 22, "White")
    canvas.draw_text(str(lives), [50, 80], 22, "White")
    canvas.draw_text(str(score), [50, 150], 22, "White")
    ...
    score += group_group_collide(missile_group, rock_group) * 5
...
def rock_spawner():
    ...
    while dist(rock_pos, my_ship.get_position()) < 4 * my_ship.get_radius():
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]    
    rock_vel = [random.choice([1, -1]) * (random.random() * .6 - .3 + score // 10), random.choice([1, -1]) * (random.random() * .6 - .3 + score // 10)]
